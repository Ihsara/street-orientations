// Builds a single-country page from data.json. All narrative is derived from the
// data itself (rankings, scores, rose shape) — no external/historical claims.
(function(){
  const page = document.getElementById("page");
  const country = page.dataset.country;
  const other = page.dataset.other;
  const slug = s => s.toLowerCase().replace(/[^a-z]/g,"");

  let DATA = null;
  let subset = "all";

  function withScore(cs){
    // Sort ascending by order_score for the active subset; null scores last.
    return cs.slice().sort((a,b)=>{
      const sa=a.order_score[subset], sb=b.order_score[subset];
      if(sa===null && sb===null) return 0;
      if(sa===null) return 1;
      if(sb===null) return -1;
      return sa - sb;
    });
  }

  function render(){
    const cs = withScore(DATA.cities.filter(c=>c.country===country));
    const others = DATA.cities.filter(c=>c.country===other);

    const present = cs.filter(c=>c.order_score[subset]!==null);
    const presentOther = others.filter(c=>c.order_score[subset]!==null);

    const mean = a => a.reduce((s,x)=>s+x,0)/a.length;
    const meanScore = present.length ? mean(present.map(c=>c.order_score[subset])) : null;
    const meanOther = presentOther.length ? mean(presentOther.map(c=>c.order_score[subset])) : null;
    const totalKm    = cs.reduce((s,c)=>s+(c.total_km[subset]||0),0);
    const totalEdges = cs.reduce((s,c)=>s+(c.edges[subset]||0),0);
    const mostGrid  = present[0]           || null;
    const leastGrid = present[present.length-1] || null;

    document.getElementById("srcline").textContent =
      `Data: OpenStreetMap drive networks, 3 km radius · ${country}, ${cs.length} cities · `;
    document.getElementById("standfirst").innerHTML =
      `${cs.length} ${country === "Finland" ? "Finnish" : "Vietnamese"} cities as 3&nbsp;km disks around their historic centres, ` +
      `ordered from <b>most grid-like</b> to <b>least</b>. Lower order score = streets cluster on fewer axes.`;

    page.innerHTML="";

    // ---- at-a-glance summary ----
    const glance=document.createElement("section");
    glance.className="glance";
    const tint = (meanOther==null || meanScore==null) ? ""
      : (meanScore < meanOther ? "tint-lo" : "tint-hi");
    const fmtScore = s => s===null ? "—" : s.toFixed(3);
    glance.innerHTML=`
      <div class="figure"><span class="num ${tint}">${fmtScore(meanScore)}</span><span class="cap">mean order score · ${subset}</span></div>
      <div class="figure"><span class="num">${Math.round(totalKm).toLocaleString()}</span><span class="cap">km of ${subset} street</span></div>
      <div class="figure"><span class="num">${mostGrid ? fmtScore(mostGrid.order_score[subset]) : "—"}</span><span class="cap">most grid-like · <b>${mostGrid ? mostGrid.name : "—"}</b></span></div>
      <div class="figure"><span class="num">${leastGrid ? fmtScore(leastGrid.order_score[subset]) : "—"}</span><span class="cap">least grid-like · <b>${leastGrid ? leastGrid.name : "—"}</b></span></div>`;
    page.appendChild(glance);

    // ---- factual narrative (data-derived only) ----
    const prose=document.createElement("section");
    prose.className="prose";
    let body;
    if(!mostGrid){
      body = `<p>No ${country} city in this lineup has any <b>${subset}</b> edges within its 3&nbsp;km radius.
        Switch the <em>Edges</em> filter back to <b>All</b> or <b>Arterials</b> to see the orientation roses.</p>`;
    } else {
      const spread = leastGrid.order_score[subset] - mostGrid.order_score[subset];
      let cmp = "";
      if(meanOther!=null){
        const diff = Math.abs(meanScore-meanOther);
        const rel = meanScore < meanOther ? "more grid-like (lower)" : "less grid-like (higher)";
        cmp = ` On average ${country}'s cities score <b>${meanScore.toFixed(3)}</b>, ${rel} than ${other}'s <b>${meanOther.toFixed(3)}</b> — a gap of ${diff.toFixed(3)}.`;
      }
      body = `
        <p>Across these ${cs.length} cities the <b>${subset}</b>-edge order score runs from <b>${mostGrid.order_score[subset].toFixed(3)}</b>
        (${mostGrid.name}, the most grid-like here) to <b>${leastGrid.order_score[subset].toFixed(3)}</b>
        (${leastGrid.name}, the least), a spread of ${spread.toFixed(3)}.${cmp}</p>
        <p>In each rose below, the longer arms mark the directions most of that city's streets run.
        Read the shape, not just the number: two strong perpendicular arms is a grid; an even ring is organic growth.</p>`;
    }
    prose.innerHTML = body;
    page.appendChild(prose);

    // ---- ranked detail roses ----
    const sec=document.createElement("section");
    sec.className="plate";
    sec.innerHTML=`
      <div class="plate-head">
        <h2>${country} · ${cs.length} cities · ${subset}</h2>
        <span class="meta">${totalEdges.toLocaleString()} edges · ${Math.round(totalKm).toLocaleString()} km</span>
      </div>
      <div class="roses detail"></div>`;
    const grid=sec.querySelector(".roses");
    cs.forEach((c,i)=>{
      const fig=cityCard(c,{rank:i+1, detail:true, subset});
      const axis=document.createElement("div");
      axis.className="stat";
      axis.textContent = c.order_score[subset]===null
        ? `no ${subset} edges in 3 km radius`
        : `streets mostly run ${dominantAxis(c.dist[subset])}`;
      fig.appendChild(axis);
      grid.appendChild(fig);
    });
    page.appendChild(sec);

    // ---- compare hook ----
    const cmpBox=document.createElement("section");
    cmpBox.className="compare";
    let line = `See how <b>${other}</b>'s cities compare.`;
    if(meanOther!=null && meanScore!=null){
      line = meanScore < meanOther
        ? `${country}'s cities are <b>more grid-like</b> on average than ${other}'s (under <em>${subset}</em>). See the contrast →`
        : `${country}'s cities are <b>less grid-like</b> on average than ${other}'s (under <em>${subset}</em>). See the contrast →`;
    }
    cmpBox.innerHTML=`
      <div class="txt">${line}</div>
      <a class="btn" href="${slug(other)}.html">${other} →</a>
      <a class="btn" href="index.html" style="background:transparent;color:var(--ink-soft);border:1px solid var(--rule)">Both side by side</a>`;
    page.appendChild(cmpBox);

    // ---- method note ----
    const note=document.createElement("aside");
    note.className="note";
    note.innerHTML=`
      <h3>How to read this</h3>
      <p>A bin's <b>radius</b> grows with the total length of road pointing that way (counted both
      ways, so every rose is symmetric). The <b>order score</b> is the streets' directional entropy
      normalised so <code>0</code> is a perfect grid and <code>1</code> is perfectly uniform.</p>
      <p>The <em>Edges</em> filter restricts the calculation to a road-class subset: <b>arterials</b>
      keeps motorway/trunk/primary/secondary (and their link variants); <b>backbone</b> keeps only
      motorway, trunk, and primary. Smaller subsets often expose a stronger grid signature.</p>`;
    page.appendChild(note);
  }

  function bindEdgesControl(){
    const buttons = document.querySelectorAll("[data-subset]");
    buttons.forEach(btn=>{
      btn.addEventListener("click", ()=>{
        subset = btn.dataset.subset;
        buttons.forEach(b=>b.classList.toggle("on", b===btn));
        render();
      });
    });
    document.querySelector('[data-subset="all"]').classList.add("on");
  }

  loadData().then(d=>{
    DATA = d;
    if(!d.cities.some(c=>c.country===country)){
      page.innerHTML='<p class="loading">No cities for '+country+' in data.json.</p>';
      return;
    }
    bindEdgesControl();
    render();
  }).catch(e=>showLoadError(page, e));
})();
